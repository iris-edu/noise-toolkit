...
def polarizationAngles(z1,z2,z3):
   ...
   for l in xrange(0,4):
      ...
      tmp   = (abs(z2)*abs(z2) * np.cos(angle + cPhase(z2)) * np.cos(angle + cPhase(z2))) + (abs(z3)*abs(z3) * np.cos(angle + cPhase(z3)) * np.cos(angle + cPhase(z3)))
      if l ==0:
         max     = tmp
         theta_h = angle
      elif(tmp > max):
   ...
   for l in xrange(0,4):
      angle = val + float(l) * math.pi / 2.0
      tmp   = abs(z1)*abs(z1)*np.cos(angle+cPhase(z1))*np.cos(angle+cPhase(z1)) + abs(z2)*abs(z2)*np.cos(angle+cPhase(z2))*np.cos(angle+cPhase(z2)) + abs(z3)*abs(z3)*np.cos(angle+cPhase(z3))*np.cos(angle+cPhase(z3))
      if l ==0:
         max     = tmp
         theta_v = angle
      elif(tmp > max):
